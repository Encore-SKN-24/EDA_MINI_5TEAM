...
- 김지원: 전처리 단계에서 데이터 구조를 이해하는 데 많은 시간이 필요했다. 팀 내 정해진 규칙을 바탕으로 JSON 취합과 스키마 통일 과정을 차근히 수행했다. 단순 구현이 아니라 분석을 위한 데이터 구조가 왜 중요한지 고민하는 계기가 되었다. 이번 경험을 토앻 데이터 전처리에 대한 기초 역량을 쌓을 수 있었다. 
