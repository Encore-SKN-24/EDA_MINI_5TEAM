...
def main():
    print('[Lotteria] Fetching store data...', flush=True)
    response_text = fetch_store_data()
    print('[Lotteria] Extracting JSON...', flush=True)
    raw_store_data = extract_store_info(response_text)
    print('[Lotteria] Transforming...', flush=True)
    transformed_data = transform_store_data(raw_store_data)
    save_to_json(transformed_data, JSON_FILE)
    print(f'[Lotteria] Data saved to {JSON_FILE}', flush=True)
...
if __name__ == "__main__":
    try:
        ...
        print(f'[Lotteria] 오류로 data/에 파일이 저장되지 않았습니다: {e}', flush=True)
        raise